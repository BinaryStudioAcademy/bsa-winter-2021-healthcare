...
const AddMemberForm: React.FC<Props> = ({ className }) => {
  ...
  const loadOptions = (inputValue: string, callback: any) => {
    dispatch(MessagesActionCreator.loadFilteredMembersAsOptions(inputValue, callback));
  };

  const onSubmit = (formData: IMemberPayload) => {
    formData.member && dispatch(MessagesActionCreator.addMember(formData.member));
    reset();
  };
